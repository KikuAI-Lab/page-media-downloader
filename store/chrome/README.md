...
Расширение работает локально, без аккаунта, сервера разработчика, аналитики, рекламы и телеметрии. После нажатия оно локально обрабатывает адрес текущей страницы и её видимое содержимое, чтобы найти медиа; эти данные не отправляются разработчику и не сохраняются как история браузера. Постоянный доступ ко всем сайтам не запрашивается: временный доступ к текущей вкладке появляется только после действия пользователя.
